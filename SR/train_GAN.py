...
from FSRGAN_net import FSRNet, Discriminator, FeatureExtractor
...
parser.add_argument('--dataset', default='helen', type=str)
...
trn_dloader = torch.utils.data.DataLoader(dataset=trn_dataset, batch_size=12, shuffle=True)
...
criterion_L1 = nn.functional.l1_loss

criterion_hinge = nn.MultiLabelMarginLoss()
...
# outputs
output_dir = '../FSRGAN-OUT/outputsGAN_{:%Y-%m-%d-%H-%M-%S}/'.format(datetime.now())
...
for epoch_idx in range(1, num_epoch + 1):
    ...
    for batch_idx, (image_lr, image_hr, hmaps, pmaps) in enumerate(trn_dloader, start=1):
        ...
        # train generator
        G.zero_grad()
        loss_f1 = a * criterion_MSE(fake, real)
        #loss_f1 = a * criterion_L1(fake, real)
        #loss_f2 = b * criterion_MSE(y_c, real)
        loss_f2 = b * criterion_MSE(y_c, real)
        #loss_f3 = c * criterion_MSE(prs, image_pr)
        #loss_f = loss_f1 + loss_f2 + loss_f3
        loss_f = loss_f1 + loss_f2
        #loss_f = loss_f3

        loss_p1 = a * criterion_MSE(F(fake), F(real).detach())
        #loss_p2 = b * criterion_MSE(F(y_c), F(real).detach())
        #loss_p = r_p * (loss_p1+loss_p2)
        loss_p = r_p * loss_p1
        #loss_pc= r_p * criterion_MSE(F(y_c), F(real).detach())  ###

        generator_content_loss = loss_f + loss_p       ####
        generator_adversarial_loss = r_c * criterion_BCE(D(fake), real_label)
        #generator_adversarial_loss = r_c * criterion_hinge(D(fake), real_label)
    ...
    for batch_idx, (image_lr, image_hr, _, _) in enumerate(val_dloader, start=1):
        ...
        if batch_idx <= 100:
            ...
            real_hrimg.save(os.path.join(_dir, '%d_hr_img.jpg' % batch_idx))
            real_lrimg.save(os.path.join(_dir, '%d_lr_img.jpg' % batch_idx))
