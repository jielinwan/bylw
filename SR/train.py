...
from original_net import FSRNet
...
trn_dloader = torch.utils.data.DataLoader(dataset=trn_dataset, batch_size=11, shuffle=True)
...
for epoch_idx in range(1, num_epoch + 1):
    ...
    for batch_idx, (image_lr, image_hr, _, _) in enumerate(val_dloader, start=1):
        ...
        if batch_idx <= 10:
            ...
            real.save(os.path.join(_dir, '%d_realimg_pred.jpg' % batch_idx))
