...
class ResBlock_WithBN(nn.Module):

    def __init__(self, dim):
        super(ResBlock_WithBN, self).__init__()
        self.layers = nn.Sequential(
            # n.BatchNorm2d(dim),
            # nn.LeakyReLU(negative_slope=0.2, inplace=True),
            nn.ReflectionPad2d(1),
...
class CoarseSRNetwork(nn.Module):
    ...
    def __init__(self):
        ...
        self.rrdb_blocks = nn.Sequential(*([RRDB(64)] * 13))
...
class FineSRDecoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(267, 64, kernel_size=3, stride=1, padding=1, bias=False),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
        )
